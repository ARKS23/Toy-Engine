...
namespace Hazel {
	...
	struct ProfileResult {
		std::string Name;						// 监测的函数名称

		FloatingPointMicroseconds Start;		// 开始时刻
		std::chrono::microseconds ElapsedTime;	// 执行耗时
	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "result.json") {
			std::lock_guard lock(m_Mutex);  // 上锁，线程安全控制
			if (m_CurrentSession) {	// 检查是否已经有会话在运行,如果有的话关掉正在运行的
			...
			if (m_OutputStream.is_open()) {
				...
				if (Log::GetCoreLogger()) {		// BeginSession可能在Log::Init之前, 保护性检查
		...
		void WriteProfile(const ProfileResult& result) {
			...
			if (m_CurrentSession) {
				...
				m_OutputStream.flush();	// 刷新缓冲区写入文件
			}
			else {
				HZ_CORE_WARN("CurrentSession is nullptr!");
			}
		}

		static Instrumentor& Get() {	// 单例模式
	...
	private:
		std::mutex m_Mutex;
		InstrumentationSession* m_CurrentSession;	// 当前会话状态指针
		std::ofstream m_OutputStream;
	};


	class HAZEL_API InstrumentationTimer {	// RAII思想进行自动化管理
	public:
		InstrumentationTimer(const char* name) : m_Name(name) , m_Stopped(false){
			m_StartTimepoint = std::chrono::steady_clock::now();	// 开始计时
		}
		...
		void Stop() {
			...
			auto highResStart = std::chrono::duration_cast<FloatingPointMicroseconds>(m_StartTimepoint.time_since_epoch());
